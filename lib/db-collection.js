...
class DbCollection {
   ...
   */
  all() {
    return duplicate(Array.from(this._records.values()));
  }
  ...
  */
   ...
   */
  _findRecord(id) {
    return this._records.get(id.toString());
  }
  ...
  _findRecordsWhere(query) {
    ...
    if (typeof query === "function") {
      ...
      filterFn = (value) =>
        keys.every((key) => String(value[key]) === String(query[key]));
    }
  ...
  _insertRecord(data) {
    ...
    this._records.set(attrs.id, attrs);
